...
int lessthan(contestant a, contestant b)
{
...
int main() {
	...
	contestant cont[MAX_CONTESTANTS + 1];
	...
	while(num_cases-- >= 0)
	{
		...
		{
			cont[i].num_solved = cont[i].tot_penalty = cont[i].ever_submitted = 0;
		...
		for(i = 1; i < MAX_CONTESTANTS; i++)
		{
			...
			for(j = i + 1; j <= MAX_CONTESTANTS; j++)
				if(lessthan(cont[perm[max]], cont[perm[j]]) == 1)
					max = j;
			temp = perm[max];
			perm[max] = perm[i];
			perm[i] = temp;
		...
		if(num_cases == 0)
			putchar('\n');
		for(i = 1; i <= MAX_CONTESTANTS; i++)
			if(cont[perm[i]].ever_submitted == 1)
				printf("%d %d %d\n", perm[i], cont[perm[i]].num_solved, cont[perm[i]].tot_penalty);
